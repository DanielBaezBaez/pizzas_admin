...
require_once("include/items_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "cerulean";
		$layout->customCssPageName = "items_detailspreview";
...
$page_layouts["items_detailspreview"] = $layout;
...
if($mastertable == "tipo_items")
{
	$formattedValue = make_db_value("tipo_item_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("tipo_item_id") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("tipo_item_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	ingredientes_principales - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ingredientes_principales", $data, $keylink);
			$row["ingredientes_principales_value"] = $value;
			$format = $pSet->getViewFormat("ingredientes_principales");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("ingredientes_principales")))
				$class = ' rnr-field-number';
			$row["ingredientes_principales_class"] = $class;
	//	url_image - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("url_image", $data, $keylink);
			$row["url_image_value"] = $value;
			$format = $pSet->getViewFormat("url_image");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("url_image")))
				$class = ' rnr-field-number';
			$row["url_image_class"] = $class;
	//	costo - Currency
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("costo", $data, $keylink);
			$row["costo_value"] = $value;
			$format = $pSet->getViewFormat("costo");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("costo")))
				$class = ' rnr-field-number';
			$row["costo_class"] = $class;
	//	detalle - 
			$viewContainer->recId = $recordsCounter;
	...
	//	tipo_item_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("tipo_item_id", $data, $keylink);
			$row["tipo_item_id_value"] = $value;
			$format = $pSet->getViewFormat("tipo_item_id");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("tipo_item_id")))
				$class = ' rnr-field-number';
			$row["tipo_item_id_class"] = $class;
	//	creado_por - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("items", "detailspreview"));
