...
if($rowcount) 
{
	...
	//	email_verified_at - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("email_verified_at", $data, $keylink);
			$row["email_verified_at_value"] = $value;
			$format = $pSet->getViewFormat("email_verified_at");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("email_verified_at")))
				$class = ' rnr-field-number';
			$row["email_verified_at_class"] = $class;
	//	password - 
			$viewContainer->recId = $recordsCounter;
