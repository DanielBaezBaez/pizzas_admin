...
require_once("include/detalles_pedidos_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "cerulean";
		$layout->customCssPageName = "detalles_pedidos_detailspreview";
...
$page_layouts["detalles_pedidos_detailspreview"] = $layout;
...
if($mastertable == "items")
{
	$formattedValue = make_db_value("item_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("item_id") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("item_id") . "=" . $formattedValue;
}
if($mastertable == "pedidos")
{
	$formattedValue = make_db_value("pedido_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("pedido_id") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("pedido_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	item_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("item_id", $data, $keylink);
			$row["item_id_value"] = $value;
			$format = $pSet->getViewFormat("item_id");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("item_id")))
				$class = ' rnr-field-number';
			$row["item_id_class"] = $class;
	//	creado_por - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("detalles_pedidos", "detailspreview"));
