...
require_once("include/detalles_facturas_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "united";
		$layout->customCssPageName = "detalles_facturas_detailspreview";
...
$page_layouts["detalles_facturas_detailspreview"] = $layout;
...
{
	$formattedValue = make_db_value("factura_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("factura_id") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("factura_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	factura_id - 
		    ...
		    $value = $viewContainer->showDBValue("factura_id", $data, $keylink);
			$row["factura_id_value"] = $value;
			$format = $pSet->getViewFormat("factura_id");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("factura_id")))
				$class = ' rnr-field-number';
			$row["factura_id_class"] = $class;
	//	empresa_id - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("detalles_facturas", "detailspreview"));
