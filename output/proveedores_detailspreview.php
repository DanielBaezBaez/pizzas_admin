...
require_once("include/proveedores_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "cerulean";
		$layout->customCssPageName = "proveedores_detailspreview";
...
$page_layouts["proveedores_detailspreview"] = $layout;
...
if($mastertable == "personas")
{
	$formattedValue = make_db_value("persona_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("persona_id") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("persona_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	persona_id - 
			...
			$format = $pSet->getViewFormat("persona_id");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
...
$xt->load_template(GetTemplateName("proveedores", "detailspreview"));
