...
function checkTableName($shortTName, $type=false)
{
	...
	if ("barrios" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ciudades" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("clientes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("detalles_facturas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("detalles_pedidos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("empresas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("estados" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("facturas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("insumos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("items" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("menus" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("paises" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pedidos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("personales" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("personas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("preparaciones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("promociones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("proveedores" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("sucursales" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_items" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tipo_personales" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	$arr = array();
		...
		$arr[]="ciudades";
		$arr[]="clientes";
		$arr[]="detalles_facturas";
		$arr[]="detalles_pedidos";
		$arr[]="empresas";
		$arr[]="estados";
		$arr[]="facturas";
		$arr[]="insumos";
...
//	default permissions
	if($table=="detalles_facturas")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="detalles_pedidos")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password, &$pageObject = null, $fireEvents = true )
{
	...
	$_SESSION["GroupID"] = "";
		...
		$_SESSION["OwnerID"] = $data["empresa_id"];
	$_SESSION["_clientes_OwnerID"] = $data["empresa_id"];
		$_SESSION["_detalles_facturas_OwnerID"] = $data["empresa_id"];
		$_SESSION["_detalles_pedidos_OwnerID"] = $data["empresa_id"];
		$_SESSION["_facturas_OwnerID"] = $data["empresa_id"];
		$_SESSION["_insumos_OwnerID"] = $data["empresa_id"];
		$_SESSION["_items_OwnerID"] = $data["empresa_id"];
		$_SESSION["_pedidos_OwnerID"] = $data["empresa_id"];
		$_SESSION["_personales_OwnerID"] = $data["empresa_id"];
		$_SESSION["_preparaciones_OwnerID"] = $data["empresa_id"];
		$_SESSION["_promociones_OwnerID"] = $data["empresa_id"];
		$_SESSION["_proveedores_OwnerID"] = $data["empresa_id"];
		$_SESSION["_sucursales_OwnerID"] = $data["empresa_id"];
		$_SESSION["_users_OwnerID"] = $data["empresa_id"];
