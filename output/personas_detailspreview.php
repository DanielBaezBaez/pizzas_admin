...
require_once("include/personas_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "united";
		$layout->customCssPageName = "personas_detailspreview";
...
$page_layouts["personas_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	cedula - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("cedula", $data, $keylink);
			$row["cedula_value"] = $value;
			$format = $pSet->getViewFormat("cedula");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("cedula")))
				$class = ' rnr-field-number';
			$row["cedula_class"] = $class;
	//	direccion - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("personas", "detailspreview"));
