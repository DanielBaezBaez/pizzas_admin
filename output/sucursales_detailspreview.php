...
require_once("include/sucursales_variables.php");
...
$page_layouts["sucursales_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	correo - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("correo", $data, $keylink);
			$row["correo_value"] = $value;
			$format = $pSet->getViewFormat("correo");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
...
$xt->load_template(GetTemplateName("sucursales", "detailspreview"));
